...
9. <bound name="reader_continuity">Scene 是内部生产单元。输出不得写“场景一/二/三”或 scene 标题；第一段须脱离标题仍能说明与上一场的因果、时间或地点关系。</bound>
</hard_constraints>

<positive_replacements rationale="负向指令易被反向激活，每条 ban 都配对'改写成什么'。">
- 不写 "他感到 X" → 改写为：人物碰到本场物件后作出选择，并让选择立刻留下可见后果；身体反应只有会妨碍行动或兑现既有线索时才写。
...
- 不写形容词标签的情绪 → 改写为：让读者从人物改变做法、处置物件或承担后果中自行判断。
...
| 3 | 前 500 字 ≥ 1 次压力反应链 | 外部压力落地 → 主角作出具体选择 → 页面出现可见后果；固定身体反应词命中不计通过。 |
...
| 8 | 反模式零容忍 | 信息倒斗（单段 > 150 字含 ≥ 2 条背景/阴谋/旧案）= 0；术语堆叠（首次出现私设词 ≤ 5）；主角不能只承受压力而不作选择。 |
...
> 验尸格的灯只剩一盏。蜡花结成黑芯，他压低灯座，火苗照出死者左手指甲缝里一粒未化的盐。
> "沈仵作。"门外有人敲了两下。"周神算的话——天亮前焚化。"
> 沈青崖用针尖挑出盐粒，折进纸角，压在验尸格编号下面。
> "嗯。"
> 烛影里那人递进一张盖了红印的纸条。沈青崖把纸条压到盐粒旁，两处红印一新一旧。门外那只手当即往回抽。
> 沈青崖用银针钉住纸角："劳烦周神算亲自来取。"

为何 PASS：
- 首句出现主角全名 + 主语动作（gate #1）。
- 第二段"外头有人敲了两下 + 焚化倒计时" → 可感冲突 + 倒计时威胁（gate #2，且在前 200 字内）。
- 焚化压力落地后，主角立即藏证、对印、扣住来人把柄，形成压力 → 选择 → 可见后果（gate #3）。
...
- 不输出任何 `#` / `##` 场景标题；内部 scene_number 不得进入读者正文。
...
- [ ] 删除所有场景标签后，首段仍能独立承接上一场；无地点瞬移、无说明时间跳跃、无悬空代词
- [ ] ≥ 1 条世界法则细节渗透
