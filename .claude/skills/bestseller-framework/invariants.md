...
- ❌ **每章字数不得低于本作品的 `platform_profile.pacing_preference.chapter_word_count` 下限**——七猫=2500，起点=3000，番茄=2000，未指定平台时默认 1800–2600–3500。低于即 `status: rework`；从场景 / 内心 / 对白扩写，不得以形容词灌水。
...
- ❌ **第一章 (first_chapter) 8 项 hard_gates 任一失败即 `status: rework`**——不论其他评分多高。具体：
  - 主角必须在前 100 字进入聚光灯
  - 前 200 字必须有可感冲突
  - 前 500 字必须出现压力 → 主角具体选择 → 可见后果；固定身体词命中不算通过
  - 章末前必须完成一次可感小爽点
  - 不允许心理独白型信息倒斗（单段 > 150 字内心戏含多条背景）
  - 不允许冷面工具人主角（连续两个压力节点都不作具体选择）
  - 不允许私设术语堆叠（首次出现 ≥ 6）
  - 章末 150 字内必须有勾子
