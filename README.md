...
[![Migrations](https://img.shields.io/badge/Alembic-32%2B%20versions-brightgreen)](migrations/versions)
[![Services](https://img.shields.io/badge/Services-130%2B%20modules-success)](src/bestseller/services)
...
- [设计哲学](#设计哲学)
- [架构设计思路与数据流](#架构设计思路与数据流)
- [能力全景](#能力全景)
- [框架级评审（2026-05-21）](#框架级评审2026-05-21)
- [架构总览](#架构总览)
- [核心子系统](#核心子系统)
...
外部写作方法论不能只变成 prompt 里的几句提醒。BestSeller 现在把方法论拆成 **来源资产 → MethodologyCard → MethodologyProfile → Contract Overlay → CheckerReport → Project Health / Repair Action** 的链路：来源可追溯，启用可配置，审稿可落库，修复可排队。当前默认策略是 `audit_only` 优先，先积累数据，再把稳定规则升级为严格门控。

---

## 架构设计思路与数据流

本节回答三件事：**为何这样分层**、**数据从哪进从哪出**、**哪些规则在代码里硬保证**。更完整的存储与 Agent 边界见 [`docs/architecture.md`](docs/architecture.md)。

### 为何这样分层（与代码的对应关系）

| 思路 | 落点 |
|:---|:---|
| **单一真值源** | 正文、规划版本、Canon、时间线、审校与 `LlmRun` 以 PostgreSQL 为权威；`output/` 下的 Markdown 等是**导出衍生物**，可随时从库中重建。 |
| **用例编排 vs 领域逻辑** | FastAPI / CLI / Worker 只负责触发与鉴权；`services/pipelines.py` 编排 `run_*_pipeline`；具体规则在 `drafts`、`knowledge`、`reviews`、`continuity` 等模块内聚。 |
| **可恢复工作流** | `WorkflowRun` / `WorkflowStepRun` 持久化每一步，支持中断续跑、SSE/Redis 进度与成本审计。 |
| **规划先于正文** | `PlanningArtifactVersion` 锁版本；近年补强 **Story Design Core**：在批量章纲之前用结构化合同约束「这本书独特什么」，见下节。 |
| **场景级事务边界** | 每个 scene 流水线末尾 `checkpoint_commit()`，避免长事务；失败重试粒度到场景而非整章。 |

### 端到端数据流（主生产链）

```
接入层 (CLI / REST :8000 / Web :8787 / ARQ Worker / MCP :3000)
        │
        ▼
创建工作流 WorkflowRun + StepRun（审计根：project_id + workflow_run_id）
        │
        ├─► 规划链：Premise → Book/World/Cast/Volume 等 PlanningArtifactVersion
        │         ↑ 可选增强：StoryDesignKernel + story_design_grammars（题材「剧情语法」）
        │         → 章纲 / SceneCard 写入 Volume → Chapter → Scene 表族
        │
        └─► 生产链（对每个 Chapter）
                 for each Scene:
                    ① context.py 装配 SceneWriterContextPacket（圣经摘要 + 合约 + RAG）
                    ② llm.complete_text(role=writer) → SceneDraftVersion
                    ③ knowledge：propagate_scene_discoveries（Canon / Timeline / Snapshot）
                    ④ reviews + 校验器（CheckerReport / 平台门控 / continuity / methodology…）
                    ⑤ 未过则 regen_loop + editor（rewrite 策略在 === reference only === 内）
                    ⑥ checkpoint_commit（场景级落盘）
                 章合并 chapter draft → 章审校 → Phase B/C/D + methodology 钩子（见 quality_gates.yaml）
                 里程碑 project consistency / repair
        │
        ▼
exports：Markdown / DOCX / EPUB / PDF → 本地或对象存储（仍非真值源）
```

**读取路径（写作当下）**：`SceneCard` + 当前 frontier 可见的 `WorldRule` + 有效 `CanonFact` + 检索 `RetrievalChunk`（pgvector + pg_trgm + 结构权重）→ 压入 token 预算 → `complete_text`。

**写入路径（场景结束后）**：新草稿版本指针、关联 `ReviewReport`/`QualityScore`、`CanonFact`/`TimelineEvent` 追加、chunk 重建队列——其中**与当前版本强一致**的写入与步骤状态在同一事务边界内提交（详见 `docs/architecture.md` 事务一节）。

### 核心实现不变量（开发 Mode A 必守）

1. **所有 LLM 调用**经 `src/bestseller/services/llm.py::complete_text`，并传入 `project_id` 与 `workflow_run_id`，自动落 `llm_runs`。
2. **评分与重写**：结构化进 `ReviewReportModel` / `QualityScoreModel`；`RewriteTask` 给 editor 的正文策略必须在 `=== reference only ===` 围栏内。
3. **知识单调性**：Canon 以追加与 supersede 为主链；时间线、倒计时等由 `continuity` 与 Phase D 检查器守卫。
4. **门控分层**：硬拦截 / 软审计+Override / 纯记录；软违规可走 Phase C 追债账本（`quality_gates.yaml` 开关）。
5. **方法论资产链**：外部方法论先进入 `data/methodology_sources/*` 和 `config/methodology_profiles/*`，再由 `methodology_runtime.py` 适配进既有 review / health，而不是直接污染 writer prompt。

### Story Design Core（规划上游合同）

在「直接生成大量章纲」之前，框架可用 **Story Design Kernel**（`story_design_kernel.py`）把一本书的**独特钩子、人物价值冲突、世界规则施压轴、宏观结构与情节树**固化为 Pydantic 合同，并与 **`config/story_design_grammars/*.yaml`**（按题材可组合的叙事语法）及 **`story_shape_router.py`（书籍形态路由）** 联动。设计意图是：让下游 `planner` / 大纲批处理吃「已锁定差异」的结构化输入，而不是默认剧情模板的隐性泄漏。集成路线见 `docs/plans/2026-05-12-story-design-core-capability-integration.md`。

### 全书级与连载级验收（结果面）

在场景/章节门控之外，代码侧还包含面向**跨章形态**的扫描与 readiness 信号，例如：`whole_book_quality_gate.py`（全书参与感/结构账本）、`ranking_readiness.py`、`commercial_planning_readiness.py`、`reverse_outline_gate.py`、`chapter_causality_gate.py` 等，与 `config/quality_gates.yaml` 及既有 `NovelScorecard` / `CheckerReport` 体系并列或聚合使用——用于上架前体检、规划期自检与 repair 队列输入，而不是替代单章流水线。
...
| **方法论资产层** | OCR/外部方法论 → Card → Profile → Prompt Block | `methodology_cards.py` · `methodology_profile.py` · `data/methodology_sources/` | ✅ |
| **方法论专项门控** | 黄金三章 · 动作场景结构 · 契诃夫强调物 | `opening_three_function_gate.py` · `action_scene_structure_gate.py` · `chekhov_emphasis_gate.py` | ✅ |
| **方法论健康度** | Coverage · Top Issues · Longform Chaos · Repair Actions | `methodology_health.py` · `methodology_runtime.py` · `project_health.py` | ✅ |
| **世界扩张** | Volume Frontier · Deferred Reveal | `world_expansion.py` | ✅ |
| **剧情设计内核** | 命题合同 · 人物/世界冲突轴 · 情节树 · 形态路由 | `story_design_kernel.py` · `story_shape_router.py` · `story_design_grammars.py` · `config/story_design_grammars/` | ✅ |
| **全书形态门控** | 跨章参与感账本 · 章间因果/逆向大纲 | `whole_book_quality_gate.py` · `chapter_causality_gate.py` · `reverse_outline_gate.py` | ✅ |
| **连载/上架信号** | 排行榜 readiness · 商业规划 readiness | `ranking_readiness.py` · `commercial_planning_readiness.py` | ✅ |
...
**结论**：当前框架已经足够全面，达到“可持续生产长篇小说”的高级工程化阶段；但还不能宣称“足够优秀到稳定产出头部商业作品”。它的强项是结构化、可审计、可恢复、可持续优化；短板集中在真实作品质量校准、榜单样本回归、人工编辑体验和规则严格度标定。

### 当前成熟度判断

| 维度 | 当前状态 | 判断 |
|:---|:---|:---|
| 端到端生产链 | Premise/规划/场景/章节/审稿/重写/导出已闭环 | ✅ 已成型 |
| 长篇一致性 | Canon、Timeline、Snapshot、Contract、Narrative Lines、Debt Ledger 均有系统承载 | ✅ 强于普通写作脚本 |
| 质量门控 | CheckerReport、Write Gate、Scorecard、Project Health、平台门控、方法论门控并存 | ✅ 体系完整 |
| 方法论吸收 | Plova 方法论已资产化并接入 prompt/review/health | ✅ 具备可扩展路径 |
| 商业化判断 | Fanqie/Qimao/readiness/榜单能力已有信号层 | ⚠️ 需要更多真实样本校准 |
| 正文表现 | 有反模板、反重复、风格/节奏/钩子检查，但仍依赖模型执行力 | ⚠️ 需通过真实章节集持续打磨 |
| 产品体验 | CLI/Web/Worker 可用，健康报告可看 | ⚠️ 编辑工作台和可解释 repair 还需加强 |
| 证据闭环 | 单测和模块回归扎实，缺少长期全书 benchmark 仪表盘 | ⚠️ 需补“作品级评测基线” |

### 框架优秀的地方

1. **分层正确**：规划、上下文、生成、审稿、知识传播、修复、导出没有混在一个 prompt 里，系统边界清楚。
2. **数据模型足够厚**：Chapter/Scene Contract、Clue/Payoff、PlotArc/ArcBeat、Canon/Timeline、ReviewReport/QualityScore 能支撑长篇迭代。
3. **质量系统可扩展**：统一 `CheckerReport` 让新 gate 可以进入同一个 scorecard / health / repair 面板。
4. **方法论接入方式健康**：新方法论先资产化、再配置、再审计，避免“把大师语录塞进 prompt”导致不可验证。
5. **默认策略稳健**：新规则先 `audit_only`，对真实生产链低风险；后续可以按误报率升级到 strict。

### 还不够优秀的地方

1. **方法论素材未 100% 闭合**：`plova.36-38` 仍是 pending source，方法论覆盖报告会如实暴露。
2. **缺少作品级质量基准**：目前测试能证明功能不坏，不能证明 30/100/500 章成品稳定好看。
3. **规则权重未充分校准**：opening/action/chekhov/chaos 已接入，但哪些应阻断、哪些只提示，需要真实样章和人工反馈标定。
4. **编辑体验仍偏工程化**：Project Health 能给 action，但还需要更面向编辑的“为什么、改哪里、怎么改、改后是否通过”的闭环界面。
5. **商业平台信号需要持续更新**：番茄/七猫/榜单规则会变，readiness 需要定期重新采样和回归。

### 下一阶段优化重点

1. 建立 `methodology_evaluation_suite`：用真实章节样本验证方法论 gate 的误报率、漏报率、修复收益。
2. 把 Project Health 升级成编辑任务台：按“风险 → 证据 → 修改建议 → 一键重跑 → 复检”组织。
3. 做 30/100 章作品级 benchmark：跟踪留钩、重复、伏笔债、角色漂移、长线失控指数。
4. 校准 gate 模式：把稳定且低误报的规则从 `audit_only` 升级为 `warn/strict`。
5. 补齐 pending 方法论来源，并把 Plova 之外的方法论按同一资产链导入，验证框架的可迁移性。
...
- **Alembic 迁移**：`migrations/versions` 下 30+ 个迁移版本，支持在线升级（具体以仓库为准）
...
config/story_design_grammars/*.yaml       ← 剧情设计语法（题材可组合）
...
| **迁移** | Alembic（`migrations/versions`，持续增长） |
...
│   │   ├── methodology_cards.py         #   方法论来源资产与卡片加载
│   │   ├── methodology_profile.py       #   方法论 profile 渲染
│   │   ├── methodology_overlay.py       #   Contract overlay 标准化
│   │   ├── methodology_runtime.py       #   CheckerReport → review/health 适配
│   │   ├── methodology_health.py        #   方法论覆盖与长篇失控指标
│   │   ├── opening_three_function_gate.py
│   │   ├── action_scene_structure_gate.py
│   │   ├── chekhov_emphasis_gate.py
...
│   │   ├── story_design_kernel.py       #   剧情设计内核合同
│   │   ├── story_design_grammars.py     #   题材剧情语法加载
│   │   ├── story_shape_router.py        #   书籍形态路由
│   │   ├── whole_book_quality_gate.py   #   全书级质量扫描
│   │   ├── exports.py                   #   多格式导出
│   │   ├── publishing/                  #   发布平台适配
│   │   └── …                            #   其余 60+ 专项模块
│   ├── infra/
...
│   ├── story_design_grammars/           # 题材剧情语法（YAML）
│   ├── methodology_profiles/            # 方法论 profile（如 plova_structured_writing_v1）
│   ├── prompt_packs/                    # 题材 prompt 包
│   ├── novel_categories/                # 题材分类
│   ├── writing_methodology.yaml         # 写作方法论
│   └── facets/                          # 配置切面
├── migrations/versions/                 # Alembic（持续增长）
│   ├── …
...
│   └── …
├── docs/                                # 设计文档
├── data/methodology_sources/            # 外部方法论来源、manifest、cards
├── examples/                            # 示例配置和规划产物
├── docker-compose.yml
├── Dockerfile                           # 多阶段构建
└── pyproject.toml
...
| [novel-generator 融合 P0 记录与验证](docs/novel-generator-fusion-p0-记录与验证-20260817.md) | 外部 skill 融合三件套：story-bible mermaid 图解 / 章节契约回执(warn-only) / 伏笔在飞存量（真机 before/after 字节级对照） |
...
| [平台安全网 P1 五件 记录与验证](docs/平台安全网-P1五件与后验诊断-记录与验证-20260804.md) | 停止意图独立成表（治「书停不掉」）+ 触发器分类重试预算（治 88 万 token 循环）+ 配置冻结核验（治章节边界漂移）+ 成本归因 + 判官第三态；9168 单测全过，契约回声假说经对照组实测证实（lift +0.107） |
| [建书设定不落地 七轮根治 记录与验证](docs/建书设定不落地-七轮根治-记录与验证-20260806.md) | 「爽文无代价」全链断 5 处 + 3 处新暴露缺陷；淘汰赛本体降权（偏好非过滤）、查重基线排除 0 章废壳、代价指令送达 7 个设计阶段、本体修复覆盖判决面；**关键发现：反母题检测器已全线退役但注释仍称其存在**；七次真机后设定首次完整落地 |
