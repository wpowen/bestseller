...
async def _revise_until_clean(
    ...
    *,
    ...
    """Generate→detect→去AI味 revise loop. Returns (cleaned_content, trace).

    Each round: run the detector, and if it flags AI-flavor spans, ask the
    writer model to rewrite ONLY the flagged sentences (keep plot/characters/
    length), guided by the same cinematic_pov rubric used at generation. Stops
    early when the detector is clean (score 0 / no spans).
    """
    ...
    for rnd in range(1, rounds + 1):
        ...
        system_prompt = (
            ...
            "你要逐条核对正文、把违反的句子改干净，但严格保持剧情/人物/字数不变"
            "（只可微增不可删减情节），只动有问题的句子，其余照搬。直接输出改写后的"
            "完整正文，不要任何解释或标注。\n\n" + rubric
        )
        user_prompt = (
            "【检测出的 AI 味问题（必须逐条消除）】\n"
            ...
            "1) 解释规则/术语/数字条款：'开卷磬，议事司点人才用'、'砚台未离身者不得入堂——堂规第卅七条'、"
            "'凡研者入堂须当场研墨一推'——一律删掉解说，让规则从人物的反应、对白、后果里透出来。\n"
            "2) 把对方的路数/算计逐环讲明：'先逼落砚，落砚即离身，离身即失势'——删，只写可见的逼迫动作。\n"
            "3) 单个模板化微动作 / 生理套路：'眼瞳一缩'、'心跳漏了一拍'、'心头一紧'、'呼吸顿了半拍'——换成具体、跟当下绑定的身体动作。\n"
        ...
        req = LLMCompletionRequest(
            ...
            prompt_version="ab-harness-revise",
            project_id=project_id,
            max_tokens_override=max(2048, int(target * 2.4)),
        )
        async with session_factory() as session:
            result = await complete_text(session, settings, req)
            await session.rollback()
        revised = (result.content or "").strip()
        if revised and len(revised) > target * 0.5:
            content = revised
        else:
            trace.append(f"round {rnd}: revise returned too short, keep previous")
            break
    final = detect(content, language="zh")
    trace.append(f"final: score {final.overall_score:.0f}, spans {len(final.spans)}")
...
async def _run(args: argparse.Namespace) -> int:
    ...
    for name, user_prompt, target in briefs:
        compiled = compile_methodology(
            ...
            language="zh-CN",
            chapter_no=1,
            token_budget=4200,  # match production
        ...
        if args.revise > 0:
            content, rtrace = await _revise_until_clean(
                session_factory, settings, project_id, content,
                target=target, rounds=args.revise,
            )
            for line in rtrace:
                print(f"    {name} revise · {line}")
...
def main() -> int:
    ...
    parser.add_argument("--revise", type=int, default=0, help="去AI味 self-review rewrite rounds after generation")
