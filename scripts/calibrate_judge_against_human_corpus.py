...
🚫 **结论：这份数据不能当基线用，它是「同源高估」的证据。**

真机结果（每层 14 章 × 2 次采样）：
    人类玄幻向 中位 0.16 · 市井/现实向 0.12 · 混合 0.19
    我们的书   端盘画神 0.35 · 破澡堂 0.45 · 验证书 0.325
我们的书比人类出版章高 2-4 倍——而读者盲评判它们「50 章零爽点」。
不可能是真的。查因：`llm_runs` 显示写手与判官**同为 MiniMax-M3**
（近 3 小时 critic 608 次 / writer 104 次，同一个 model_name）。
判官在给自家家族的产物打分，本项目早已记过这条（同源高估 0.15-0.20），
这里的偏差比记录的还大。

因此两把尺子各有一个已定量的局限，刚好互补：

    尺子              跨来源可比(vs人类)      题材公平
    确定性词表分类器    ✓ 无模型偏好           ✗ 玄幻73%/市井24%
    语义判官           ✗ 同源高估             ✓

  · 跟人类比 → 只能用确定性分类器 + 题材分层
  · 我们的书前后比 → 只能用语义判官（同源偏差两边抵消）

本脚本的价值因此**不是产出基线，而是量出同源偏差有多大**。
要拿判官跟人类比，必须换一个不同家族的模型当判官。
