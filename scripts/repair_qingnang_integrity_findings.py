...
""",
    7: """【第7章重跑指令】
门禁提示开篇楼层/层级锚点混乱，且出现“剩余玩家”等游戏化漂移词。重跑时保留本章主线，但把空间锚点改成一个清晰场景：如果是楼层，就只保留一个楼层；如果是下行层级，就避免写成真实楼层冲突。开头必须先交代当前位置、目标、阻碍，再推进动作。所有游戏化称谓改成受困者、入局者、欠账人或回执名额。
""",
    9: """【第9章重跑指令】
门禁提示章节开头已经启动后又出现新的时间地点重启。重跑时删掉二次开头，让“凌晨三点零八分”这类时间锚点只作为同一场戏中的消息触发，不要把读者拉回另一个开篇。结尾继续强化第10/11章点击钩。
""",
    10: """【第10章重跑指令｜拼接开场修复】
门禁提示章节已经在太平间走廊启动后，又用“凌晨三点的太平间灯管……”重启了一次开场。重跑时必须删掉二次开场，只保留一条连续场景线：快递单/铜镜碎片 -> “替认”二字 -> 陈默到场 -> 镜债逼问 -> 林渊发现替认已收。不要重复“小雨为什么能活到现在”和快递单信息。章节体量控制在2000-3000字，结尾必须把“替认已收。三百账起。”推到第11章，而不是停在解释句。
""",
    11: """【第11章重跑指令｜正典污染修复】
修复目标：删除“陈守正”旧版陈家线姓名，避免陈默线被错误人物吞掉。
必须执行：
- “陈守正”不得作为正典人物、牌位或旧名出现；如果当前段落需要陈家旧债，只能改写成“陈默父亲旧账”“陈家旧名残页”或未确认的旧账代号。
- 不新增陈家势力，不把陈默线提前扩成家族线。
- 保留本章已有的认账/否认规则推进，并让线索回到十七栋、陈默、小雨或林家父亲旧案。
""",
