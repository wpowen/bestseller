...
DESLOP_DISCOURSE_CATEGORIES = frozenset(
    {
        ...
        # 沉默应答族复读(没答话/没接话/没吭声,2026-08-13 用户终审「没答话/没xx
        # 的ai味描述还是有」)：人类出版章 1042 章 mean 0.049/万字、p99 1.86，
        # 我们的稿 25-28×、11-13% 章过门槛（2026-08-30 复核）；patcher 无静态
        # 替换（沉默镜头要换成具体反应），只有整段重写能清。
        "silent_reply_spam",
        "staccato_saturation",
        ...
        "inanimate_agency",
        # 语料级词频放大(2026-08-20《罚我守坟》: 全书「方向」417 次 8.10/千字,
        # ch20 达 23.38 —— 人类 400 篇出版章的最大值只有 2.65)。patcher 无静态
        # 替换(要判断每一处该换成什么说法或直接删),只有整段重写能拆。
        # 阈值来自语料而非词表: 「密度超过人类语料该词最高记录」的词数 ≥8,
        # 留出 343 篇人类章误报 0/343。去掉该 finding 分数确实降 4.0,
        # 即重写清掉它会被采纳判据认出来(不会重蹈「抓到了不修」)。
        "corpus_overamplified",
        # （debt_metaphor_leak 曾在此：检测器 2026-08-02 退役恒返回 []——
        #  「母题警察自伤」定案——2026-08-30 死链清理时从触发集移除。）
        ...
        "dialogue_famine",
        # ── 2026-08-30 去AI味融合批（17 个外部仓库调研 + 本仓语料校准准入，
        # 见 docs/deai-fusion-design-20260830.md 与 patterns_zh.json 各规则 why）。
        # 七条全部 advisory 封顶、无杀权，进触发集只为拿定向重写：
        # 每条都是 patcher 无静态替换的句式/结构腔。
        # 罐头反应镜头（AI 6.5×）：通用零件拼情绪，要换成绑定当下的具体反应。
        "stock_reaction",
        # 「了一下」微动作复读（AI 2.6×）：要并句删次，不是换字。
        "micro_action_tic",
        # 反序对比排比 是X不是Y（AI 11×）：要把辨认写成具体来源，不是删否定词。
        "reverse_contrast",
        # 音量反差腔（聚集才报）：要删铺垫写实效。
        "voice_contrast",
        # 章末预告/盖章收尾（末600字窗口，人类≈0）：要换成具体事实钩子。
        "trailer_ending",
        "trailer_summary",
        # 相邻句同构串（人类 0/1135）：要改变信息进入方式，patcher 动不了句形。
