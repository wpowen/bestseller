...
# ── 主线上升感（前进轨迹）检测 ───────────────────────────────────────────────
# 榜单简介几乎都让读者看到"这书往哪走、爽点会怎样升级"（上升阶梯 / 终极目标 /
# 复仇统治轨迹）。纯气氛、只有开局氛围、看不到主角要往哪走的简介 = 主线模糊，
# 列表页留不住人（真机《我在命馆收诡当租客》主线模糊是同一失败模式）。
# 刻意做【广】：任一上升/复仇/统治/目标信号在场即通过——floor 只兜底"完全零轨迹"
# 的纯气氛稿；弱轨迹（如只写"做成城里最硬的"）的质量由生成 prompt ③c + LLM 判官管，
# 不在这里硬判，以把误伤压到近零。
_ASCENT_SIGNAL_RE = re.compile(
    # 上升阶梯 / 范围推进
    r"从[^，。,.！!？?]{0,16}(?:到|做到|升到|爬到|一路|成为|干到|打到)"
    r"|越[^，。,.]{0,8}越"
    r"|一路|层层|步步|一步步|节节|一级一级|一步步"
    # 终极目标 / 登顶
    r"|最终|终将|终究|迟早|直到"
    r"|登上|爬到|问鼎|封神|封王|称霸|一统|横扫|杀穿|巅峰|之巅|顶点|顶端"
    r"|最强|第一人|天下第一|最高处|规则改写权"
    # 复仇 / 统治 / 目标兑现（爽文轨迹）
    r"|逆袭|翻盘|崛起|做大|越做越|越战越|越来越强"
    r"|让[^，。,.！!？?]{0,16}(?:跪|求他|臣服|俯首|付出代价|后悔)"
    r"|买下[^，。,.]{0,6}(?:城|整座|全)"
    r"|碾压|打脸|镇住|镇压|拿下|夺下|夺回|夺取|复仇"
    # 规模扩张名词阶梯
    r"|(?:街区|商圈|城市|全城|全国|天下)[^，。,.]{0,4}(?:级|第一|最)"
)


def _has_progression_signal(text: str) -> bool:
    """True when the blurb shows any forward trajectory / ascent / goal."""

    return bool(text) and bool(_ASCENT_SIGNAL_RE.search(text))
...
def evaluate_blurb_appeal(
    *,
    ...
    # 主线模糊，列表页留不住人。selling_triad 只查身份+冲突+代价，看不见"轨迹"，
    # 于是气氛稿也能达标（真机《命馆》主线模糊照样过门）。这里做保守兜底：整份
    # 简介(标题+正文+premise)完全没有任何上升/复仇/统治/目标信号 → 封顶到
    # progression_cap(<68)，触发重生补上升感。弱轨迹质量由生成 prompt ③c 管。
    # 豁免：情感两难/切肤/迫近/骇异型强简介（悬疑/怪谈/代价流）靠戏剧张力驱动，
    # 本就不靠上升阶梯——它们有"主线"（一个迫近的抉择），只是形态不是升级。有任一
    # 具身情绪范畴在场即视为主线可见，不封顶。只有既无上升轨迹又无戏剧两难的纯气氛
    # 稿才判主线模糊。
    progression_cap = float(bar_cfg.get("progression_cap", 66))
    has_trajectory = _has_progression_signal(combined) or bool(
        _embodied_emotion_categories(combined)
    )
    if not has_trajectory and total > progression_cap:
        total = progression_cap
        findings.append(
            "[主线上升感缺失] 简介只有开局氛围，看不到主角要往哪走、爽点会怎样升级"
            f"（无上升阶梯/终极目标/复仇统治轨迹）→ 主线模糊、列表页留不住人 → 封顶 {progression_cap:.0f}"
        )
        suggestions.append(
