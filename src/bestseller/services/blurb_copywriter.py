...
def _build_candidate_messages(
    ...
    *,
    ...
    user = (
        ...
        "写，简介许了不一样的诺，读者点进来就是上当。细节可以少写，不许改写；"
        # 2026-08-25《吃我一筷》定罪：故事核里有完整机制（刻纹→吸灵气→回流喂
        # 自己），简介写手拿着「可以少写」把机制整个删光，只剩效果炸裂——
        # 读者不知道菜为什么能崩人护体。「少写」的下限必须钉死。
        "但金手指『做什么→为什么因此得什么』这一句话因果必须在简介前半交代一次，"
        "只有效果、机制零交代的简介是废稿。\n"
        "⑨d【首现带锚】每个物件、规矩、事件第一次出现时必须自带一口来历"
        "（一个短语就够：谁弄的、为什么在这）——读者没读过故事核，"
        "凭空出现又凭空被处置的东西读起来就是断层。\n"
    ...
    *,
    ...
    if champion is not None:
        ...
        needs_polish = (
            ...
            # 教学轴逻辑病（机制矛盾/无锚指代/论据不撑论点/效果无因）不杀候选，
