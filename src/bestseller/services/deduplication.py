    ...
    def _flush_cluster() -> None:
        ...
        findings.append({
            ...
            "message": (
                ...
                f"最高覆盖率 {best[3]:.0%}）。不可直接删除——需走 rewrite_task "
                f"定向重写后簇。"
                # ⚠️ 这里原本硬编码断言「疑似多场景卡共享同一 cut_point 导致高潮
                # 被完整重写两遍」。本函数是纯文本检测器，**无从知道这本书用不用
                # 场景管线**，那句因果是猜的。真机（书 9，2026-08-24）当场证伪：
                # 88 张场景卡全都有 cut_point、同章共享 0 组，而且这本书是整章模式、
                # 0 份场景稿——场景卡根本没参与正文生成。那句话把我的排查带偏了一分钟。
                # 检测器只陈述它观察到的证据，不断言它验证不了的原因。
            ),
        })
