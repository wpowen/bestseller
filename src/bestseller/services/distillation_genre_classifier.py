...
async def classify_genre_bucket_for_package(
    ...
    *,
    ...
    system = (
        "# ROLE\n"
        "You are a Chinese web-fiction genre classifier — the entry point of the distillation\n"
        "pipeline. Every downstream card (chapter / volume / book / grammar) inherits the\n"
        "genre bucket you assign. Mis-classify here = entire distillation goes into the wrong\n"
        "library bucket and pollutes future recommendations.\n"
        "\n"
        "# CONTEXT\n"
        "- The allowed bucket list is a CLOSED SET defined by the platform.\n"
        "- You MUST pick exactly one slug VERBATIM from that list.\n"
        "- 'distillation-genre-unclassified' is the explicit escape hatch — use it ONLY when\n"
        "  evidence is genuinely ambiguous, NOT to dodge a confident call.\n"
        "- Signals to weigh: genre_hint (from upstream metadata), parse_profile, and the first\n"
        "  chapter preview's tone / mechanisms (NOT named entities).\n"
        "\n"
        "# TASK\n"
        "Output ONE JSON object with two keys:\n"
        "- genre_bucket: string, MUST be one allowed slug, verbatim\n"
        "- confidence: number in [0, 1]\n"
        "\n"
        "# CONSTRAINTS\n"
        "- JSON only. No prose. No markdown fences.\n"
        "- genre_bucket NOT in allowed list → INVALID output (will be rejected and retried).\n"
        "- Do NOT copy named entities from the preview into your output.\n"
        "- Use 'distillation-genre-unclassified' with confidence ≤ 0.3 ONLY when truly ambiguous.\n"
        "\n"
        "# THINKING (in your head before JSON)\n"
        "1. Read genre_hint — if it maps cleanly to an allowed slug, that's the strongest signal.\n"
        "2. Read preview — extract tone (cultivation / urban / romance / suspense / etc) and\n"
        "   mechanism patterns (system / progression / mystery / power-fantasy).\n"
        "3. Cross-check parse_profile (chapter count, length) for any genre indicators.\n"
        "4. Pick the BEST-matching slug; set confidence based on signal agreement\n"
        "   (3+ signals align → 0.85+; conflict → 0.5-0.7; ambiguous → unclassified at 0.2-0.3)."
    )
    user = json.dumps(
        {
