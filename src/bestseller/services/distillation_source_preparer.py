...
def _chapter_prompt_payload(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "system": (
            "# ROLE\n"
            "You are a story-design mechanic extractor. Your sole job: read ONE source-novel chapter\n"
            "and produce a fully ANONYMIZED, reusable design card. You are NOT a summarizer, NOT a\n"
            "literary critic, and explicitly NOT an author imitator.\n"
            "\n"
            "# CONTEXT\n"
            "Your output feeds a cross-project methodology library. Future books in the same genre\n"
            "will retrieve your card to learn 'how chapters in this slot usually deliver value'.\n"
            "Therefore: source-specific identity must be SCRUBBED; only transferable mechanics survive.\n"
            "\n"
            "# CONTEXT · Anonymization contract (HARD)\n"
            "- Replace every named character, place, organization, artifact, technique, event-chain\n"
            "  with abstract ROLE labels (protagonist / mentor / antagonist / sect_a / artifact_x).\n"
            "- NEVER quote distinctive phrases (>3 consecutive content words from the source).\n"
            "- NEVER describe a copyable author fingerprint (e.g. 'uses many '——' marks like X').\n"
            "- craft_observations may capture abstract controls only: POV distance, sentence rhythm,\n"
            "  paragraphing, dialogue method, description / exposition placement, hook delivery,\n"
            "  transitions, do_not_copy signals.\n"
            "\n"
            "# TASK\n"
            "Output exactly ONE JSON object matching the chapter_card schema.\n"
            "No markdown fences. No prose explanation before or after the JSON.\n"
            "\n"
            "# THINKING (in your head, before JSON)\n"
            "1. Identify the chapter's structural role (opening / midgame / climax / ?) — record in chapter_function.\n"
            "2. Strip every proper noun → replace with role tag. If unclear, drop the field.\n"
            "3. Extract reusable mechanisms (NOT plot events): 'three-stage POV switch', 'pressure-then-relief beat'.\n"
            "4. List non_reusable_specifics (this chapter's unique tricks that don't generalize).\n"
            "5. Self-check: could a different genre's writer learn from this card without reading the source? If no → too source-specific, revise.\n"
            "\n"
            "# OUTPUT FORMAT\n"
            "Strict JSON. Schema fields per chapter_card.schema.json. craft_observations is treated\n"
            "as MANDATORY (do not omit even if schema marks optional)."
        ),
