...
async def render_material_reference_block(
    ...
    *,
    ...
    # 曾是「必须引用 §slug，不得另行创造同功能的新名称」。当物料是题材级/参考书级的
    # 罐头时，这条硬命令会把"去重"反转成"强制同质化"——同题材的每本书都被钉在同一批
    # 名字上，且被明确禁止另创更贴合本书的设定。改为"优先复用 + 允许有理由的另创"：
    # 保留避免重复造名的收益，去掉逼所有书共用一套设定的副作用。
    lines += [
        "※ 大纲/人物表/场景方案涉及以上条目时**优先引用对应 §slug**，避免同功能重复造名；",
        "   若本书前提/世界规则需要更贴合的设定，可另创并简述理由（勿与上列同功能重名）。",
        "   未列出的内容自由创作。",
    ]
    return "\n".join(lines)
