...
_PLAYBOOKS: dict[str, QualityRepairPlaybook] = {
    ...
    "HYPE_MISSING": QualityRepairPlaybook(
        code="HYPE_MISSING",
        scope="chapter",
        # 判据来自三类爽文读者（番茄免费男频/起点付费老书虫/女频）独立盲评后的
        # **唯一共识**——三人在憋屈耐受、反派智商下限、是否接受「安静地赢」上
        # 全都相反，只有这三条完全一致。
        # ⚠️ 种词铁律：只写**结构**，不给任何爽点 token（打脸/跪下/求饶/碾压
        # 一个都不出现）。用户 2026-08-16 明确指出的病正是「把这些当最优解注入
        # 系统，导致每本书必定长成同一副样子」。给结构，模型自己找本书的说法。
        instruction=(
            "本章读不出兑现——读者会觉得白读了一章。补一次真正的结算，三件必须同时成立："
            "①对象是**一个有名字的人**，且他在本章之前具体地轻视、阻挠或亏欠过主角"
            "（「某个势力」「一群人」不算）；"
            "②现场至少有一个第三方**看见并因此改变**，把他的具体动作、原话或待遇变化写出来"
            "（「众人震惊」「一片哗然」这类概括等于没写）；"
            "③结算完主角手上**多一样能说出名字、下一章还能用的东西**"
            "（实物、权限、身份、一句公开许诺、某个人的效忠皆可）。"
            "不要新增设定或临时给主角新能力来完成它——只用本章之前已经建立过的"
            "资源、关系或信息；用不上就说明该结算的对象选错了，换一个。"
        ),
        acceptance=(
            "本章能指认出：谁被结算（有名字）、谁看见了并改变了（有具体动作或原话）、"
            "主角带走了什么（能说出名字）。三者缺一即未达标。"
            "且结算所用的手段在本章之前已经出现过，没有新增设定。"
        ),
    ),
