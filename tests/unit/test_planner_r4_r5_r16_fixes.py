...
* **R5 — cross-batch consumed-event ledger**: progressive outline batches
  ...
  existing repair-directive retry path (soft-accept on the final attempt).
...
class TestConsumedEventLedger:
    ...
    @pytest.mark.asyncio
    async def test_repair_loop_soft_accepts_residual_duplicate_on_final_attempt(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        project = build_project()
        settings = build_settings()
        ...
        # The duplicate must not abort the book — soft-accept and flag it.
        unresolved = payload["_meta"]["event_dedup_unresolved"]
        assert unresolved and unresolved[0]["chapter_number"] == 11
        assert any(
            item.get("status") == "passed_with_event_duplicates" for item in history
        )
